class novice_blackjump extends Phaser.Scene {
    constructor() {
      super('novice_blackjump');
    ...
    create(){
      ...
      decor_avant1 = this.physics.add.image(0,0,'decor_avant').setImmovable(true).setSize(15360,100).setOffset(0,980).setOrigin(0,0).setDepth(-3);
      decor_avant1.body.setAllowGravity(false);
      decor_avant2 = this.add.image(3840,0,'decor_avant').setOrigin(0,0).setDepth(-3);
      decor_avant3 = this.add.image(7680,0,'decor_avant').setOrigin(0,0).setDepth(-3);
      decor_avant4 = this.add.image(11520,0,'decor_avant').setOrigin(0,0).setDepth(-3);

      /*contrôles*/cursors = this.input.keyboard.createCursorKeys();
      perso = this.physics.add.sprite(1500,875,'course')/*.setCircle(100,-30,0)*/.setSize(90,200);
      this.anims.create({
        key: 'anim_course',
      ...
      // win/lose
      ecran_victoire = this.add.sprite(960,540,'win').setDepth(5).setScrollFactor(0,0).setVisible(false);
      this.anims.create({
        key: 'anim_win',
      ...
      ecran_defaite = this.add.sprite(960,540,'lose').setDepth(5).setScrollFactor(0,0).setVisible(false);
      this.anims.create({
        key: 'anim_lose',
      ...
      // obstables
      bloc11 = this.physics.add.image(3500,300,'bloc1').setDepth(-2).setOrigin(0,0).setSize(200,200).setOffset(47,39).setImmovable(true);
      ...
      bloc21 = this.physics.add.image(5300,350,'bloc2').setDepth(-2).setOrigin(0,0).setSize(400,200).setOffset(45,46).setImmovable(true);
      bloc21.body.setAllowGravity(false);
      this.physics.add.collider(perso,bloc21);

      bloc22 = this.physics.add.image(6500,740,'bloc2').setDepth(-2).setOrigin(0,0).setSize(400,200).setOffset(45,46).setImmovable(true);
      bloc22.body.setAllowGravity(false);
      this.physics.add.collider(perso,bloc22);

      bloc32 = this.physics.add.image(7300,540,'bloc3').setDepth(-2).setOrigin(0,0).setSize(200,400).setOffset(46,43).setImmovable(true);
    ...
    update(){
      ...
      if (isEnd == 1 && perso.x > 8100 && loopfalse == 0) {
        isEnd=0;
        loopfalse = 1;
        this.physics.pause();
      ...
      if (isEnd == 1 && perso.x < 7500 && loopfalse == 0) {
        isEnd=0;
        loopfalse = 1;
        this.physics.pause();
